...
export async function apiRequest(path, config, signal, options = {}) {
    const baseUrl = (config.baseUrl ?? "http://127.0.0.1:8000").replace(/\/$/, "");
    const token = config.token ?? process.env.BIBLIOTECA_OPENCLAW_TOKEN;
    if (!token)
        throw new Error("Falta BIBLIOTECA_OPENCLAW_TOKEN para consultar Biblioteca Kindle");
...
export default defineToolPlugin({
    ...
    tools: (tool) => [
        ...
        tool({
            ...
            execute: ({ query, limit }, config, context) => apiRequest(`/works?q=${encodeURIComponent(query)}&limit=${limit ?? 8}`, config, context.signal),
        }),
        tool({
        ...
        tool({
            ...
            execute: ({ work_id, profile_id, title }, config, context) => apiRequest(`/works/${encodeURIComponent(work_id)}/conversations`, config, context.signal, { method: "POST", body: { profile_id, title: title ?? "" } }),
        }),
        tool({
        ...
        tool({
            ...
            execute: ({ conversation_id, personal_note_ids, annotation_ids }, config, context) => apiRequest(`/conversations/${encodeURIComponent(conversation_id)}/context`, config, context.signal, { method: "PUT", body: { personal_note_ids, annotation_ids } }),
        }),
        tool({
            ...
            execute: ({ conversation_id, query, search_scope, search_work_ids }, config, context) => apiRequest(`/conversations/${encodeURIComponent(conversation_id)}/library-search`, config, context.signal, { method: "POST", body: { search_query: query, search_scope: search_scope ?? "library", search_work_ids: search_work_ids ?? [] } }),
        }),
        tool({
            ...
            execute: ({ conversation_id, ...body }, config, context) => apiRequest(`/conversations/${encodeURIComponent(conversation_id)}/turns`, config, context.signal, { method: "POST", body: { search_scope: "library", search_library: true, ...body } }),
        }),
        tool({
            ...
            execute: ({ turn_id, content }, config, context) => apiRequest(`/turns/${encodeURIComponent(turn_id)}/complete`, config, context.signal, { method: "POST", body: { content } }),
        }),
    ],
});
