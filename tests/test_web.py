...
class WebTests(unittest.TestCase):
    ...
    def test_work_detail_and_annotation_api(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn('id="book-load-feedback"', page_text)
            ...
            self.assertIn('id="new-conversation-dialog"', page_text)
            self.assertIn('id="open-new-conversation"', page_text)
            self.assertIn('id="mobile-material-toggle"', page_text)
            self.assertIn('id="focus-book-context"', page_text)
            ...
            self.assertIn("function revealAssistantResponse", book_script)
            self.assertIn("function cancelResponseAnimation({finish = false} = {})", book_script)
            self.assertIn("cancelResponseAnimation({finish: true})", book_script)
            self.assertIn("const CHAT_BOTTOM_TOLERANCE = 48", book_script)
            self.assertIn("function setupConversationScroll(container)", book_script)
            self.assertIn("function disposeConversationScroll()", book_script)
            self.assertIn("function scrollConversationToBottom({force = false} = {})", book_script)
            self.assertIn("resumeConversationAutoFollow()", book_script)
            self.assertIn("scrollConversationToBottom();", book_script)
            self.assertIn("function setCompanionFocus(active)", book_script)
            self.assertIn("function exitCompanionFocus()", book_script)
            self.assertIn('const memoryTab = document.querySelector(\'[data-book-tab="memory"]\');', book_script)
            self.assertIn("if (memoryTab) memoryTab.click();", book_script)
            self.assertIn('document.querySelector("#exit-companion-focus")?.addEventListener', book_script)
            self.assertIn("memoryTab?.scrollIntoView({block: \"start\"});", book_script)
            self.assertIn("function openNewConversationDialog()", book_script)
            self.assertIn("function showBookLoadError(error)", book_script)
            self.assertIn("loadBook().catch(showBookLoadError);", book_script)
            self.assertIn('setCompanionFocus(selected === "companion")', book_script)
            ...
            self.assertIn("function openCompanionActionsDialog()", book_script)
            self.assertIn("function filterCompanionActions()", book_script)
            self.assertIn("closeCompanionActionsForSelection();", book_script)
            stylesheet = (Path(__file__).parents[1] / "src/biblioteca_kindle/static/css/app.css").read_text()
            self.assertIn('id="open-companion-actions"', page_text)
            self.assertIn('id="companion-actions-dialog"', page_text)
            self.assertIn('id="companion-action-search"', page_text)
            self.assertIn('id="companion-action-draft"', page_text)
            ...
            self.assertIn(".companion-actions-dialog[open] { display: grid;", stylesheet)
            ...
            self.assertIn("#context-dialog[open] { height: 100dvh; }", stylesheet)
            self.assertIn("#context-form { height: 100%; max-height: none; }", stylesheet)
            self.assertIn("max-height: min(36rem, calc(100dvh - 2rem));", stylesheet)
            self.assertIn(".new-conversation-form .dialog-actions { position: sticky;", stylesheet)
            self.assertIn("body.is-companion-focused > footer { display: none; }", stylesheet)
            self.assertNotIn("body.is-companion-focused footer { display: none; }", stylesheet)
            self.assertIn("body.is-companion-focused .book-tabs { display: none; }", stylesheet)
            self.assertNotIn(".book-page:has(#panel-companion:not([hidden]))", stylesheet)
    ...
    def test_reading_conversation_can_be_created_and_messages_saved(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("✦ Explorar", page.get_data(as_text=True))
            ...
            self.assertIn('id="companion-actions-dialog"', page.get_data(as_text=True))
